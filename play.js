...
// ---- 참여자 화면 타이머 ----
function startPlayerTimer(seconds, endTime) {
    stopPlayerTimer();
    const wrap = document.getElementById('p-timer-wrap');
    ...
    function tick() {
        ...
        if (remain <= 5) {
            num.classList.add('p-timer-urgent');
        } else {
...
function startListening() {
    ...
    gameRef.on('value', snap => {
        const st = snap.val();
        if (!st || st.phase === 'lobby') { stopPlayerTimer(); pShow('wait'); return; }
        renderPlay(st);
    });
}
...
function renderPlay(st) {
    if (st.phase === 'question') {
        ...
        // 타이머 시작
        if (st.endTime && st.timeLimit) {
            startPlayerTimer(st.timeLimit, st.endTime);
        } else if (st.endTime) {
            const elapsed = Date.now() - (st.endTime - 30000);
            startPlayerTimer(30, st.endTime);
        }
        box.innerHTML = '';
